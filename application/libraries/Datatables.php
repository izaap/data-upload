  ...
  class Datatables
  {
        ...
        if(isset($_POST['sSearch_' . $i]) && $this->ci->input->post('sSearch_' . $i) != '' && in_array($mColArray[$i], $columns))
        {
          ...
          foreach($miSearch as $val)
          {
            ...
            elseif(!empty($sRangeSeparator) && preg_match("/(.*)$sRangeSeparator(.*)/i", trim($val), $matches))
            {
              ...
              if(!empty($matches[1]))
                $rangeQuery = 'STR_TO_DATE(' . $this->select[$mColArray[$i]] . ",'%W , %d-%m-%Y %H:%i:%s') >= STR_TO_DATE('" . $matches[1] . " 00:00:00','%d-%m-%Y %H:%i:%s')";

              if(!empty($matches[2]))
                $rangeQuery .= (!empty($rangeQuery)? ' AND ': '') . 'STR_TO_DATE('. $this->select[$mColArray[$i]] . ",'%W , %d-%m-%Y %H:%i:%s') <= STR_TO_DATE('" . $matches[2] . " 23:59:59','%d-%m-%Y %H:%i:%s')";
    ...
    */
    private function produce_output($output, $charset)
    {
      ...
      {
        if(strcmp($class_name,"scheduler") === 0 && strcmp($class_name,"scheduler") === 0)
        {
           $decoded_data = json_decode($row_val['excution_start']);
           //print '<pre>';
           //print_r($decoded_data); 
           //echo $decoded_data->exe_start;
           //echo $decoded_data->exp_date;
           //echo $decoded_data->exe_interval;                                 
           $row_val['excution_start'] = $decoded_data->exe_start;
           $row_val['expiry_date'] = $decoded_data->exp_date;
           $row_val['exe_interval'] = $decoded_data->exe_interval;
        }
